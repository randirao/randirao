      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          {techCategories.map((category, categoryIndex) => (
            <TechStackCard key={categoryIndex} category={category} />
          ))}
        </div>

        <div className="mt-12 p-6 bg-gradient-to-r from-sky-50 to-yellow-200 rounded-2xl shadow-md border border-stone-200">
          <h4 className="text-lg text-slate-800 mb-4 text-center flex items-center justify-center gap-2">
        ...
        <div className="mt-12">
          ...
          <div className="space-y-8">
            {technicalStrengths.map((strength, index) => (
              <TechnicalStrengthCard key={index} strength={strength} />
            ))}
          </div>
        </div>
